function out_coef = conv_deim_new(ucoef, rom_data, method)
    ...
    f_p = (rom_data.u_p(:, 2:end) * ucoef(2:end)) .* (rom_data.ux_p(:, 2:end) * ucoef(2:end)) + ...
          (rom_data.v_p(:, 2:end) * ucoef(2:end)) .* (rom_data.uy_p(:, 2:end) * ucoef(2:end));
    ...
    switch lower(method)
        ...
        case 'mcls'
            ...
            rhs = (rom_data.nl_bas_p' * f_p) + (rom_data.alpha * rom_data.tau * rom_data.mu);
        ...
        otherwise
            error('Invalid method. Use "deim", "cls", or "mcls".');
    end
