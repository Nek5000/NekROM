...
c-----------------------------------------------------------------------
      ...
      if (nio.eq.0) write (6,*) 'inside setbases_gram'
...
c          call vnorm(ub(1,1),vb(1,1),wb(1,1))
c        else
c           call opcopy(ub,vb,wb,uic,vic,wic)
         endif

         if (ifrom(2)) then
            do i=1,ns
               call copy(tb(1,i),ts0(1,i),n)
c              call sub2(tb(1,i),tb(1,0),n)
c              do j=1,ns
c                 call add2s2(tb(1,i),ts0(1,j),evec(j,i,2),n)
c              enddo
            enddo
c           call snorm(tb(1,1))
         endif
      ...
      ! only load the anchor point pressure
      ! we do not need to solve for pressure in ROM
      call copy(pb(1,0),ps(1,1),n)
c     call snorm(pb(1,0),n)
