...
c    $            snapt(1,ldim,i),ub,vb,wb)
     ...
     $            uvwb(1,1,i),uvwb(1,2,i),uvwb(1,ldim,i))
         ...
         if (ifrom(2)) then
            n=lx1*ly1*lz1*nelt
            call pod(tb(1,nb+1),eval,ug,us0,1,
     $         'H10',nb,ns,ifpb,'ops/gt2 ')

            do i=nb+1,nb*2
               do j=1,i-1
                  a=glsc3(tb(1,j),tb(1,i),bm1,n)
                  call add2s2(tb(1,i),tb(1,j),-a,n)
               enddo
               a=glsc3(tb(1,i),tb(1,j),bm1,n)
               a=1./sqrt(a)
               call cmult(tb(1,i),a,n)
            enddo
         endif
...
c                 sc2=op_glsc2_wt(
c    $               uvwb(1,1,jb),uvwb(1,2,jb),uvwb(1,ldim,jb),
c    $               uvwb(1,1,jb),uvwb(1,2,jb),uvwb(1,ldim,jb),bm1)

                  sc=sc1
c                 if (nio.eq.0) write (6,*) ib,jb,sc2,'sc2'
