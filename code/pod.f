...
c           enddo
     ...
     $            tb(1,1),eval2,ug,ts0,1,ips,nb,ns,ifpb,'ops/gt  ')
      ...
      ! z = \zeta
      ! iaug = 1: Pi'Pi_incomprn {\tilde{u} \cdot \nabla u}
      ! iaug = 2: Pi'Pi_incomprn {u \cdot \nabla u}
      ! iaug = 3: f(u)-augmentation
      ! iaug = 4: f(sqrt(lambda) z)-augmentation
      ! iaug = 5: Pi_incomprn {z_0 \cdot \nabla z + z \cdot \nabla z_0}
      ! iaug = 6: iaug = 5 + iaug = 7
      ! iaug = 7: Pi_incomprn {z \cdot \nabla z}
      ! iaug = 8: Pi_incomprn {T \hat{z}}
      ! iaug = 9: iaug = 7 + iaug = 8
      ! iaug = 10: f(u)-POD augmentation
      ! iaug = 13: iaug = 7 + buoyancy
     ...
     $            uvwb(1,1,nb+1),uvwb(1,2,nb+1),uvwb(1,ldim,nb+1),rtmp1)
               ...
               call evalcflds(flucv,uvwb(1,1,nb+1),uvwb(1,1,nb+1),
     $            ldim,1,.true.)
     ...
     $            upup,uvwb(1,1,i),tb(1,i),1,1,.true.)
      ...
      if (iaug.eq.8) then
         ...
         if (ifrom(1)) then
            do i=0,nb
               call opzero(upvp(1,1,1),upvp(1,2,1),upvp(1,ldim,1))
               call copy(upvp(1,ldim,1),tb(1,i),nv)
               call col2(upvp(1,ldim,1),bm1,nv)
     ...
     $            upup(1,1,1),upup(1,2,1),upup(1,ldim,1),bm1))
               ...
               call opcopy(
     $            uvwb(1,1,i+2*nb+2),
     $            uvwb(1,2,i+2*nb+2),uvwb(1,ldim,i+2*nb+2),
     $            upup(1,1,1),upup(1,2,1),upup(1,ldim,1))

               call opcopy(
     $            ub(1,i+2*nb+2),vb(1,i+2*nb+2),wb(1,i+2*nb+2),
     $            upup(1,1,1),upup(1,2,1),upup(1,ldim,1))
            enddo
         endif
     ...
     $            upup(1,1,1),upup(1,2,1),upup(1,ldim,1),bm1))
               ...
               call opcopy(
     $            uvwb(1,1,i+nb+1),uvwb(1,2,i+nb+1),
     $            uvwb(1,ldim,i+nb+1),
     $            upup(1,1,1),upup(1,2,1),upup(1,ldim,1))
     ...
     $            upup(1,1,1),upup(1,2,1),upup(1,ldim,1))
            ...
            do i=0,nb
c              call evalcflds(
c    $            upvp,uvwb(1,1,i),uvwb(1,1,i),ldim,1,.true.)

               call opcopy(upvp(1,1,1),upvp(1,2,1),upvp(1,ldim,1),
     $                     zeros,tb(1,i),zeros)

               call opcol2(upvp(1,1,1),upvp(1,2,1),upvp(1,ldim,1),
     $                     bm1,bm1,bm1)
